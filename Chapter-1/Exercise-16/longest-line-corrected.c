...
int main() {
  ...
  while ((len = obtainline(line, MAXLINE)) > 0) {
    // If real length of string is > MAXLINE, then read up until MAXLINE - 1.
    // https://github.com/cdude996/The-C-Programming-Language/blob/master/chapter01/1-16.c
    if (len == MAXLINE - 1) {
      lenOfChunk = len;

      // This will add multiples of (MAXLINE - 1) to len until len < MAXLINE,
      // which will correspond to the last chunk of the line and will not go
      // through this if but the next one.
