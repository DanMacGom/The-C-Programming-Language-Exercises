 ...
 * In a two's complement number representation, integers range from -(2^(n-1)) to 
 * 2^(n-1) - 1. So if we input n = -2(^(n-1)) and flip its sign, it's going to
 * integer overflow (outputs garbage). 
*/
